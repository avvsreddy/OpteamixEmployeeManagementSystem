...
namespace OpteamixEmployeeManagementSystem.Data.Repository
{
    public class ReportRepository : IReportRepository
    {
        ...
        {
            return await _context.Tasks.AsNoTracking().CountAsync();
        }
        public async Task<ProjectStatusSummaryDto> GetProjectStatusSummaryAsync()
        {
            var projects = await _context.Projects.AsNoTracking().GroupBy(p => p.Status).Select(g => new
            {
                Status = g.Key,
                Count = g.Count()
            })
            ...
            .FirstOrDefault(p => p.Status == "Active")?.Count ?? 0,

                InactiveProjects = projects
            .FirstOrDefault(p => p.Status == "Inactive")?.Count ?? 0,

                CompletedProjects = projects
            .FirstOrDefault(p => p.Status == "Completed")?.Count ?? 0
            };
        }
